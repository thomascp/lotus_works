...
void __put_char(char *p,int num){
	while(*p&&num--){
		...
		Uart_SendByte(0, (int)*p++);
	};
}

void __put_char_int(char *p,int num){
	while(*p&&num--){
		//*(volatile unsigned int *)0xd0000020=*p++;
		Uart_SendByteInt(0, (int)*p++);
	};
}
...
void printk_int(const char *fmt, ...)
{
	va_list args;
	unsigned int i;
	char print_buf[256];
...
void sprintf(char *sbuf, const char *fmt, ...)
{
	va_list args;
	unsigned int i;
	char print_buf[256];
	...
	strcpy(sbuf, print_buf);
}

msg *g_print_msg_id = NULL;
void task_print_entry()
{
#define TASK_PRINT_BUF_LEN 256
	UINT32 ret = 0;
	char msg_buf[TASK_PRINT_BUF_LEN] = {0};

	g_print_msg_id = msg_create(1024);

	while(1)
	{
		ret = msg_rcv(g_print_msg_id, 0, msg_buf, TASK_PRINT_BUF_LEN);
		if(ret < 0)
		{
			printk("msg rcv error %d\r\n", ret);
		}

		task_delay(1);

		printk("[%d\t] ", task_gettick());
		printk(msg_buf);
	}
}

void start_print_task()
{
	task_create("print_task", task_print_entry, 0, 0, 150);
}
...
void ltprintf(const char *fmt, ...)
{
	...
	if(NULL == g_print_msg_id)
	{
		printk("mod is not started\r\n");
	}
	...
	ret = msg_send(g_print_msg_id, print_buf, strlen(print_buf)+1);
	if(ret < 0)
	{
		printk("msg send error %d\r\n", ret);
	}
}
